...
//   timerBtn.classList.add("btn", "btn-dark");
//   loginInForm.appendChild(timerBtn);
...
if (localStorage.getItem("currentAcc")) {
  currentAcc = JSON.parse(localStorage.getItem("currentAcc"));
  userMessage.innerText = `Welcome back, ${currentAcc.accName.split(" ")[0]} `;
  section.classList.remove("hidden");
  usnEl.style.display = "none";
  pinEl.style.display = "none";
  loginInForm.querySelector("input[type='submit']").style.display = "none";
  ...
  timerInterval = setInterval(() => {
    ...
    timerBtn.textContent = `${String(minutes).padStart(2, "0")}:${String(
      seconds
    ).padStart(2, "0")}`;
